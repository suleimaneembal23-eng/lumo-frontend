...
const AdminLayout = () => {
    ...
    // 🔔 POLLING DE NOVOS PEDIDOS (A cada 30 segundos)
    useEffect(() => {
        const checkNewOrders = async () => {
            try {
                ...
                const res = await fetch("http://localhost:5000/api/orders?limit=1", {
                ...
                if (data && data.length > 0) {
                    ...
                    if (lastOrderWebId && latestOrderId !== lastOrderWebId) {
                        ...
                        notification.open({
                            ...
                            description: `Cliente: ${latestOrder.shippingAddress?.fullName || 'Cliente'} - €${latestOrder.totalPrice}`,
    ...
    return (
        <Layout style={{ minHeight: "100vh" }}>
            <Sider collapsible collapsed={collapsed} onCollapse={setCollapsed}>
                ...
                >
                    {!collapsed ? <span style={{ color: "white", fontWeight: "bold", fontSize: 18 }}>PAINEL SR</span> : <span style={{ color: "white", fontWeight: "bold" }}>SR</span>}
                </div>
                <Menu
                    theme="dark"
                    mode="inline"
